...
class RelicPDF:
    def __init__(self, base_dir, statusbar, relic_images):
        # Base executable directory
        self.base_dir = base_dir
        self.statusbar = statusbar

        # Image class
        self.relic_images = relic_images
    ...
    def generate_relic_pdf(self, output_name, relic, form_check):
        """
        ...
        output_path = f'{self.base_dir}output/relics/{output_name}.pdf'
        ...
        self.fill_pdf(self.base_dir + 'resources/RelicTemplate.pdf', output_path, data_dict, form_check)

        # Apply relic art to relic card
        position = {'page': 1, 'x0': 90, 'y0': 150, 'x1': 346, 'y1': 406}
        art_success = True

        # Try local path first
        try:
            self.add_image_to_pdf(output_path, relic.art_path, position)
        except:
            art_success = False
        ...
        if art_success is False:
            try:
                ...
                response = requests.get(relic.art_path, stream=True)
                img = Image.open(response.raw)
                img.save(self.base_dir + 'output/relics/temporary_relic_image.png')
                self.add_image_to_pdf(output_path, self.base_dir + 'output/relics/temporary_relic_image.png', position)
                art_success = True
            except:
        ...
        # If no URL/File given or invalid paths, then sample a relic
        if art_success is False:
            self.relic_images.sample_relic_image()
            self.add_image_to_pdf(output_path, self.base_dir + 'output/relics/temporary_relic_image.png', position)
